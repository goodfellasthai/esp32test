...
// Ensure LoRa generic only compiles if it is a LoRa device and no Node is selected
#if ((defined(WYMAN_LORAV2) || defined(WYMAN_LORAV3) ) && (!defined(WY_NODE_MASTER) && !defined(WY_NODE_DRONE) && !defined(WY_NODE_01) ))
...
#endif // Ensure LoRa generic only compiles if it is a LoRa device and no Node is selected