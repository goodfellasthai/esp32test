...
void SPI_Init()
{
  // Creates the input for SD as well as the output for TFT
  SPI.begin(TFT_SCLK,SD_MISO,TFT_MOSI); 
}

void LCD_WriteCommand(uint8_t Cmd)  
{ 
  SPI.beginTransaction(SPISettings(TFT_SPI_FREQUENCY, MSBFIRST, SPI_MODE0));
...
{ 
  if (TFT_HORIZONTAL) {
    ...
    LCD_WriteData(Xstart + TFT_OFFSET_X);
    LCD_WriteData(Xend >> 8);
    LCD_WriteData(Xend + TFT_OFFSET_X);
    
    // set the Y coordinates
    LCD_WriteCommand(0x2B);
    LCD_WriteData(Ystart >> 8);
    LCD_WriteData(Ystart + TFT_OFFSET_Y);
    LCD_WriteData(Yend >> 8);
    LCD_WriteData(Yend + TFT_OFFSET_Y);
  }
  else {
    // set the X coordinates
    LCD_WriteCommand(0x2A);
...
{
  ledcAttach(TFT_BL, TFT_PWM_FREQUENCY, TFT_PWM_RESOLUTION);
