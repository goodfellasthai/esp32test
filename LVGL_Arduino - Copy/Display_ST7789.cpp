...
void SPI_Init()
{
  SPI.begin(PIN_NUM_SCLK,PIN_NUM_MISO,PIN_NUM_MOSI); 
}
...
{ 
  ...
  digitalWrite(PIN_NUM_LCD_DC, LOW); 
  SPI_WRITE(Cmd);
...
{ 
  SPI.beginTransaction(SPISettings(SPIFreq, MSBFIRST, SPI_MODE0));
  digitalWrite(PIN_NUM_LCD_CS, LOW);  
  digitalWrite(PIN_NUM_LCD_DC, HIGH);  
  SPI_WRITE(Data);  
  digitalWrite(PIN_NUM_LCD_CS, HIGH);  
  SPI.endTransaction();
}    
...
void LCD_Reset(void)
{
  digitalWrite(PIN_NUM_LCD_CS, LOW);       
  delay(50);
  digitalWrite(PIN_NUM_LCD_RST, LOW); 
  delay(50);
  digitalWrite(PIN_NUM_LCD_RST, HIGH); 
  delay(50);
}
void LCD_Init(void)
{
  pinMode(PIN_NUM_LCD_CS, OUTPUT);
  pinMode(PIN_NUM_LCD_DC, OUTPUT);
  pinMode(PIN_NUM_LCD_RST, OUTPUT); 
  Backlight_Init();
  SPI_Init();
...
{
  ledcAttach(PIN_NUM_BK_LIGHT, Frequency, Resolution);    // 设置通道
  ledcWrite(PIN_NUM_BK_LIGHT, 100);                        // 设置亮度   
}
...
void Set_Backlight(uint8_t Light)                        //
{
  ...
  if(Light > 100 || Light < 0)
    ...
    ledcWrite(PIN_NUM_BK_LIGHT, Backlight);
  }
}
